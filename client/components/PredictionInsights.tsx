...
export default function PredictionInsights({ results }: { results: PredictionData }) {
  ...
  const [aiSource, setAiSource] = useState<"grok" | "gemini" | null>(null);
...
Return ONLY valid JSON (no other text):
{
  ...
  "risks": ["insight 1", "insight 2"]
      ...
      console.log("🤖 Calling Groq API via backend...");
      ...
      // Check if Groq was actually used
      const source = data.source || "unknown";
      if (!data.isAIGenerated) {
        console.warn("⚠️ Warning: AI API was not used, falling back to local insights");
      } else {
        console.log(`✅ ${source.toUpperCase()} API was successfully used`);
        setAiSource(source as "grok" | "gemini");
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-2 justify-between">
        ...
        <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
          ...
          {isAIGenerated ? `🤖 AI-Generated (${aiSource?.toUpperCase()})` : "❌ Using Rule-Based Analysis"}
        </div>
      </div>
      ...
      <Card className="p-4 bg-primary/5 border-primary/20">
        <p className="text-xs text-muted-foreground">
          💡 <strong>Tip:</strong> These insights are {isAIGenerated ? `AI-generated by ${aiSource?.toUpperCase()}` : "based on rule-based analysis"} based on your prediction data.
